...
class ChooseHistoryHomeworkLogic extends GetxController {
  ...
  void getHistoryListActionPage(dynamic schoolClassId, int teacherId,
      int actionPage, int page, int pageSize,String endDate,String start) async {
    var cache = await JsonCacheManageUtils.getCacheData(
            JsonCacheManageUtils.HomeworkHistoryResponse,labelId: schoolClassId.toString()+actionPage.toString()+endDate+start)
        .then((value) {
      if (value != null) {
    ...
    HomeworkHistoryResponse list =
        await homeworkRepository.getHistoryHomeworkActionPage(
            schoolClassId, teacherId, actionPage, page, pageSize,endDate,start);
    if (page == 1) {
      JsonCacheManageUtils.saveCacheData(
          JsonCacheManageUtils.HomeworkHistoryResponse,
          labelId: schoolClassId.toString()+actionPage.toString()+endDate+start,
          list.toJson());
    }
